function f = SRMQ1C2cfaRGGB(IMAGE,Tc,part)
% -------------------------------------------------------------------------
...
% SRMQ1C2cfaRGGB separates 3D cooccurrences in red pixels from blue and 
% green pixels. Directional symmetry is not applied. Each submodel contains 
% three cooccurrence features, for red, green, and blue pixels.   
...
%             F = symfea([Fmin Fmax]',Tc,3,'mnmx')';   % including rgb<->bgr symm
            F = Symm2split([Fmin Fmax],Tc,3,'minmax');   % skipped directional symmetry
        else
...
%             F = Symm2split(Fold,Tc,3,'dirsign');    % including rgb<->bgr symm
            F = Symm2split(Fold,Tc,3,'sign');   	% skipped directional symmetry
        else
%             F = symm1(Fold',2,4)';
...
for j=1:3
    D_red{j}  =  D{j}(2:2:end,2:2:end);
    D_green{j}= [D{j}(2:2:end,1:2:end),D{j}(1:2:end,2:2:end)];
    D_blue{j} =  D{j}(1:2:end,1:2:end);
end

switch type
case 'hor'
    for j=1:3
        fj{j} = [Cooc1(D_red{j},order,type,T); Cooc1(D_green{j},order,type,T); Cooc1(D_blue{j},order,type,T)];
    end
    f = fj{1}+fj{2}+fj{3};
case 'ver'
    for j=1:3
    ...
    f = fj{1}+fj{2}+fj{3};
case 'col'
    f = [Cooc1(D_red,order,type,T); Cooc1(D_green,order,type,T); Cooc1(D_blue,order,type,T)];
end
...
% length(f) is three time more due to splitting residuals to red, green, 
% and blue pixels
le = length(f); 
if mod(le,3), error('This feature is not a collection of three cooccurrence arrays'), end
switch type
    case 'minmax'               % 6 cooccurrence arrays have to be parsed
        le1 = le/6;
        f1 = [f(1:le1),        f(3*le1+1:4*le1)];     % 1st and 4th
        f2 = [f(le1+1:2*le1),  f(4*le1+1:5*le1)];     % 2nd and 5th
        f3 = [f(2*le1+1:3*le1),f(5*le1+1:end)];       % 3rd and 6th
    case 'minmaxdir'
        le1 = le/6;
        f1 = [f(1:le1),        f(3*le1+1:4*le1)];     % 1st and 4th
        f2 = [f(le1+1:2*le1),  f(4*le1+1:5*le1)];     % 2nd and 5th
        f3 = [f(2*le1+1:3*le1),f(5*le1+1:end)];       % 3rd and 6th
    otherwise
        f1 = f(1:le/3);   f2 = f(le/3+1:2*le/3);   f3 = f(2*le/3+1:end); 
end
f_merged = [Symm2(f1,T,order,type),Symm2(f2,T,order,type),Symm2(f3,T,order,type)];
