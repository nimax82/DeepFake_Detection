function f = SRMQ1CFAred(IMAGE,Tc,part,red)
% -------------------------------------------------------------------------
...
%         red ..... position of red CFA, 1 - in upper left corner, 2 - in
%         2nd row 1st column, 3 - in 2nd column, 4 - in 2nd row and column
...
f = post_processing(all1st3rdc(X,1,Tc,'1st',red),'f1',1,f,Tc);   % 1st order, q=1
% f = post_processing(all1stc(X,2,Tc,red),'f1',2,f,Tc); % 1st order, q=2
for q=[1], f = post_processing(all2ndc(X,q*2,Tc,red),'f2',q,f,Tc); end    % 2nd order
for q=[1], f = post_processing(all1st3rdc(X,q*3,Tc,'3rd',red),'f3',q,f,Tc); end 	% 3rd order
for q=[1], f = post_processing(all3x3c(X,q*4,Tc,red),'f3x3',q,f,Tc); end  % 3x3
for q=[1], f = post_processing(all5x5c(X,q*12,Tc,red),'f5x5',q,f,Tc); end % 5x5

if strcmp(part,'all')       % do all submodels
% SRM or merged SRM for color image, T=2 fixed
f = post_processing(all1st3rd(X,1,'1st',red),'f1',1,f);   % 1st order, q=1
% f = post_processing(all1st(X,2,red),'f1',2,f); % 1st order, q=2
for q=[1], f = post_processing(all2nd(X,q*2,red),'f2',q,f); end    % 2nd order
for q=[1], f = post_processing(all1st3rd(X,q*3,'3rd',red),'f3',q,f); end 	% 3rd order
for q=[1], f = post_processing(all3x3(X,q*4,red),'f3x3',q,f); end  % 3x3
for q=[1], f = post_processing(all5x5(X,q*12,red),'f5x5',q,f); end % 5x5
...
[M N three] = size(X);  [T,order] = deal(2,4);
    switch resorder
        case '1st'
         [I,J] = deal(2:M-1,2:N-1); red=5-red;  % swap red for blue
        case '3rd'
...
% spam14h/v -- to be symmetrized as spam, directional, hv-nonsymmetrical
 g.spam14h = CoocCFA(Rq,order,'hor',T,red) + CoocCFA(Uq,order,'ver',T,red);
 g.spam14v = CoocCFA(Rq,order,'ver',T,red) + CoocCFA(Uq,order,'hor',T,red);
...
for j=1:three
 ...
 g.min22h = CoocCFA(RL_min,order,'hor',T,red) + CoocCFA(UD_min,order,'ver',T,red);
 g.max22h = CoocCFA(RL_max,order,'hor',T,red) + CoocCFA(UD_max,order,'ver',T,red);
...
% minmax22v -- to be symmetrized as mnmx, directional, hv-nonsymmetrical. Good with higher-order residuals! Note: 22h is bad (too much neighborhood overlap).
 g.min22v = CoocCFA(RL_min,order,'ver',T,red) + CoocCFA(UD_min,order,'hor',T,red);
 g.max22v = CoocCFA(RL_max,order,'ver',T,red) + CoocCFA(UD_max,order,'hor',T,red);
...
for j=1:three
 ...
 g.min24 = CoocCFA(vercat(vercat(RU_min,RD_min),vercat(LU_min,LD_min)),order,'hor',T,red) + ...
     CoocCFA(horcat(horcat(RU_min,RD_min),horcat(LU_min,LD_min)),order,'ver',T,red);
 g.max24 = CoocCFA(vercat(vercat(RU_max,RD_max),vercat(LU_max,LD_max)),order,'hor',T,red) + ...
     CoocCFA(horcat(horcat(RU_max,RD_max),horcat(LU_max,LD_max)),order,'ver',T,red);
...
for j=1:three
 ...
 g.min34h = CoocCFA(vercat(Uq_min,Dq_min),order,'hor',T,red) + CoocCFA(horcat(Lq_min,Rq_min),order,'ver',T,red);
 g.max34h = CoocCFA(vercat(Uq_max,Dq_max),order,'hor',T,red) + CoocCFA(horcat(Rq_max,Lq_max),order,'ver',T,red);

% minmax34v -- v works well, h does not, to be symmetrized as mnmx, directional, hv-nonsymmetrical
 g.min34v = CoocCFA(horcat(Uq_min,Dq_min),order,'ver',T,red) + CoocCFA(vercat(Rq_min,Lq_min),order,'hor',T,red);
 g.max34v = CoocCFA(horcat(Uq_max,Dq_max),order,'ver',T,red) + CoocCFA(vercat(Rq_max,Lq_max),order,'hor',T,red);
...
for j=1:three
 ...
 g.min41 = CoocCFA(R_min,order,'hor',T,red) + CoocCFA(R_min,order,'ver',T,red);
 g.max41 = CoocCFA(R_max,order,'hor',T,red) + CoocCFA(R_max,order,'ver',T,red);
    clear R_min R_max
...
for j=1:three
 ...
 g.min34 = CoocCFA(vercat(vercat(RU_min,RD_min),vercat(LU_min,LD_min)),order,'hor',T,red) + ...
     CoocCFA(horcat(horcat(RU_min,RD_min),horcat(LU_min,LD_min)),order,'ver',T,red);
 g.max34 = CoocCFA(vercat(vercat(RU_max,RD_max),vercat(LU_max,LD_max)),order,'hor',T,red) + ...
     CoocCFA(horcat(horcat(RU_max,RD_max),horcat(LU_max,LD_max)),order,'ver',T,red);
...
for j=1:three
 ...
 g.min48h = CoocCFA(vercat(vercat(RU_min2,LD_min2),vercat(RD_min3,LU_min3)),order,'hor',T,red) + ...
     CoocCFA(horcat(horcat(RD_min2,LU_min2),horcat(RU_min3,LD_min3)),order,'ver',T,red);
 g.min48v = CoocCFA(vercat(vercat(RD_min2,LU_min2),vercat(RU_min3,LD_min3)),order,'hor',T,red) + ...
     CoocCFA(horcat(horcat(RU_min2,LD_min2),horcat(RD_min3,LU_min3)),order,'ver',T,red);
 g.max48h = CoocCFA(vercat(vercat(RU_max2,LD_max2),vercat(RD_max3,LU_max3)),order,'hor',T,red) + ...
     CoocCFA(horcat(horcat(RD_max2,LU_max2),horcat(RU_max3,LD_max3)),order,'ver',T,red);
 g.max48v = CoocCFA(vercat(vercat(RD_max2,LU_max2),vercat(RU_max3,LD_max3)),order,'hor',T,red) + ...
     CoocCFA(horcat(horcat(RU_max2,LD_max2),horcat(RD_max3,LU_max3)),order,'ver',T,red);
...
for j=1:three
 ...
 g.min54 = CoocCFA(vercat(vercat(RU_min4,LD_min4),vercat(RD_min5,LU_min5)),order,'hor',T,red) + ...
     CoocCFA(horcat(horcat(RD_min4,LU_min4),horcat(RU_min5,LD_min5)),order,'ver',T,red);  
 g.max54 = CoocCFA(vercat(vercat(RU_max4,LD_max4),vercat(RD_max5,LU_max5)),order,'hor',T,red) + ...
     CoocCFA(horcat(horcat(RD_max4,LU_max4),horcat(RU_max5,LD_max5)),order,'ver',T,red);
 
%**** FUNCTION ****%
function g = all1st3rdc(X,q,T,resorder,red)
%
...
% spam14h/v -- to be symmetrized as spam, directional, hv-nonsymmetrical
 g.spam14c = CoocCFA(Rq,3,'col',T,red) + CoocCFA(Uq,3,'col',T,red);

% minmax22h -- to be symmetrized as mnmx, directional, hv-nonsymmetrical.
for j=1:three
 [RL_min{j},UD_min{j},RL_max{j},UD_max{j}] = deal(min(Rq{j},Lq{j}),min(Uq{j},Dq{j}),max(Rq{j},Lq{j}),max(Uq{j},Dq{j}));
end
 g.min22c = CoocCFA(RL_min,3,'col',T,red) + CoocCFA(UD_min,3,'col',T,red);
 g.max22c = CoocCFA(RL_max,3,'col',T,red) + CoocCFA(UD_max,3,'col',T,red);
...
for j=1:three
 ...
 g.min24c = CoocCFA(vercat(vercat(RU_min,RD_min),vercat(LU_min,LD_min)),3,'col',T,red);
 g.max24c = CoocCFA(vercat(vercat(RU_max,RD_max),vercat(LU_max,LD_max)),3,'col',T,red);
...
% minmax34v -- v works well, h does not, to be symmetrized as mnmx, directional, hv-nonsymmetrical
 g.min34hvc = CoocCFA(horcat(Uq_min,Dq_min),3,'col',T,red) + CoocCFA(vercat(Rq_min,Lq_min),3,'col',T,red);
    clear Uq_min Dq_min Rq_min Lq_min 
 g.max34hvc = CoocCFA(horcat(Uq_max,Dq_max),3,'col',T,red) + CoocCFA(vercat(Rq_max,Lq_max),3,'col',T,red);
    clear Uq_max Dq_max Rq_max Lq_max
...
for j=1:three
 ...
 g.min41c = CoocCFA(R_min,3,'col',T,red);      clear R_min
 g.max41c = CoocCFA(R_max,3,'col',T,red);      clear R_max
...
for j=1:three
 ...
 g.min34c = CoocCFA(vercat(vercat(RU_min,RD_min),vercat(LU_min,LD_min)),3,'col',T,red);
 g.max34c = CoocCFA(vercat(vercat(RU_max,RD_max),vercat(LU_max,LD_max)),3,'col',T,red);
...
for j=1:three
 ...
 g.min48c = CoocCFA(vercat(vercat(RU_min2,LD_min2),vercat(RD_min3,LU_min3)),3,'col',T,red) +...
     CoocCFA(horcat(horcat(RD_min2,LU_min2),horcat(RU_min3,LD_min3)),3,'col',T,red);
 g.max48c = CoocCFA(vercat(vercat(RU_max2,LD_max2),vercat(RD_max3,LU_max3)),3,'col',T,red) +...
     CoocCFA(horcat(horcat(RD_max2,LU_max2),horcat(RU_max3,LD_max3)),3,'col',T,red);
...
for j=1:three
 ...
 [RU_max5{j},RD_max5{j},LD_max5{j},LU_max5{j}] = deal(max(RU_max3{j},LU{j}),max(RD_max3{j},RU{j}),max(LD_max3{j},RD{j}),max(LU_max3{j},LD{j}));
        ...
        clear RU_max2 RD_max2 LD_max2 LU_max2 RU_max3 RD_max3 LD_max3 LU_max3 
 g.min54c = CoocCFA(vercat(vercat(RU_min4,LD_min4),vercat(RD_min5,LU_min5)),3,'col',T,red) +...
     CoocCFA(horcat(horcat(RD_min4,LU_min4),horcat(RU_min5,LD_min5)),3,'col',T,red);
 g.max54c = CoocCFA(vercat(vercat(RU_max4,LD_max4),vercat(RD_max5,LU_max5)),3,'col',T,red) +...
     CoocCFA(horcat(horcat(RD_max4,LU_max4),horcat(RU_max5,LD_max5)),3,'col',T,red);
...
[M N three] = size(X); [T,order] = deal(2,4); red=5-red;
...
% spam12h/v
 g.spam12h = CoocCFA(Yh,order,'hor',T,red) + CoocCFA(Yv,order,'ver',T,red);
 g.spam12v = CoocCFA(Yh,order,'ver',T,red) + CoocCFA(Yv,order,'hor',T,red);

% minmax21
for j=1:three
 [Dmin{j},Dmax{j}] = deal(min(Yh{j},Yv{j}),max(Yh{j},Yv{j}));
end
 g.min21 = CoocCFA(Dmin,order,'hor',T,red) + CoocCFA(Dmin,order,'ver',T,red);
 g.max21 = CoocCFA(Dmax,order,'hor',T,red) + CoocCFA(Dmax,order,'ver',T,red);

% minmax41   
for j=1:three
 [Dmin2{j},Dmax2{j}] = deal(min(Dmin{j},min(Yd{j},Ym{j})),max(Dmax{j},max(Yd{j},Ym{j})));
end
 g.min41 = CoocCFA(Dmin2,order,'hor',T,red) + CoocCFA(Dmin2,order,'ver',T,red);
 g.max41 = CoocCFA(Dmax2,order,'hor',T,red) + CoocCFA(Dmax2,order,'ver',T,red);
   	clear Dmin2 Dmax2
...
for j=1:three
 ...
 g.min24h = CoocCFA(vercat(RU_min2,RD_min2),order,'hor',T,red) + CoocCFA(horcat(RU_min3,LU_min3),order,'ver',T,red);
 g.min24v = CoocCFA(horcat(RU_min2,RD_min2),order,'ver',T,red) + CoocCFA(vercat(RU_min3,LU_min3),order,'hor',T,red);
 g.max24h = CoocCFA(vercat(RU_max2,RD_max2),order,'hor',T,red) + CoocCFA(horcat(RU_max3,LU_max3),order,'ver',T,red);
 g.max24v = CoocCFA(horcat(RU_max2,RD_max2),order,'ver',T,red) + CoocCFA(vercat(RU_max3,LU_max3),order,'hor',T,red);
...
for j=1:three
 ...
 g.min32 = CoocCFA(vercat(RU_min,RD_min),order,'hor',T,red) + CoocCFA(horcat(RU_min,RD_min),order,'ver',T,red);
 g.max32 = CoocCFA(vercat(RU_max,RD_max),order,'hor',T,red) + CoocCFA(horcat(RU_max,RD_max),order,'ver',T,red);

%**** FUNCTION ****%
function g = all2ndc(X,q,T,red)
%
% X must be a matrix of doubles or singles (the image) and q is the 
% quantization step (any positive number).
%
...
% spam12h/v
 g.spam12c = CoocCFA(Yh,3,'col',T,red) + CoocCFA(Yv,3,'col',T,red);

% minmax21
for j=1:three
 [Dmin{j},Dmax{j}] = deal(min(Yh{j},Yv{j}),max(Yh{j},Yv{j}));
end
 g.min21c = CoocCFA(Dmin,3,'col',T,red);
 g.max21c = CoocCFA(Dmax,3,'col',T,red);

% minmax41   
for j=1:three
 [Dmin2{j},Dmax2{j}] = deal(min(Dmin{j},min(Yd{j},Ym{j})),max(Dmax{j},max(Yd{j},Ym{j})));
end
 g.min41c = CoocCFA(Dmin2,3,'col',T,red);      clear Dmin2
 g.max41c = CoocCFA(Dmax2,3,'col',T,red);      clear Dmax2
...
for j=1:three
 ...
 g.min24c = CoocCFA(vercat(RU_min2,RD_min2),3,'col',T,red) + CoocCFA(horcat(RU_min3,LU_min3),3,'col',T,red);
 g.max24c = CoocCFA(vercat(RU_max2,RD_max2),3,'col',T,red) + CoocCFA(horcat(RU_max3,LU_max3),3,'col',T,red);
...
for j=1:three
 ...
 g.min32c = CoocCFA(vercat(RU_min,RD_min),3,'col',T,red);
 g.max32c = CoocCFA(vercat(RU_max,RD_max),3,'col',T,red);
...
[M N three] = size(X);   [T,order] = deal(2,4); red=5-red;   % swapping NINI with IIII 
...
for j=1:three
 ...
 g.spam11 = CoocCFA(Y,order,'hor',T,red) + CoocCFA(Y,order,'ver',T,red);
...
% spam14h,v  not bad, directional, hv-nonsym, to be symmetrized as spam
 g.spam14v = CoocCFA(horcat(Yu,Yb),order,'ver',T,red) + CoocCFA(vercat(Yl,Yr),order,'hor',T,red);
 g.spam14h = CoocCFA(vercat(Yu,Yb),order,'hor',T,red) + CoocCFA(horcat(Yl,Yr),order,'ver',T,red);
...
for j=1:three
 ...
 g.min24 = CoocCFA(horcat(horcat(Dmin1,Dmin2),horcat(Dmin3,Dmin4)),order,'ver',T,red) + ...
     CoocCFA(vercat(vercat(Dmin1,Dmin2),vercat(Dmin3,Dmin4)),order,'hor',T,red);
 g.max24 = CoocCFA(horcat(horcat(Dmax1,Dmax2),horcat(Dmax3,Dmax4)),order,'ver',T,red) + ...
     CoocCFA(vercat(vercat(Dmax1,Dmax2),vercat(Dmax3,Dmax4)),order,'hor',T,red);
...
for j=1:three
 ...
 g.min22h = CoocCFA(UEq_min,order,'hor',T,red) + CoocCFA(REq_min,order,'ver',T,red);
 g.min22v = CoocCFA(UEq_min,order,'ver',T,red) + CoocCFA(REq_min,order,'hor',T,red);
 g.max22h = CoocCFA(UEq_max,order,'hor',T,red) + CoocCFA(REq_max,order,'ver',T,red);
 g.max22v = CoocCFA(UEq_max,order,'ver',T,red) + CoocCFA(REq_max,order,'hor',T,red);
...
for j=1:three
 ...
 g.min41 = CoocCFA(Dmin5,order,'ver',T,red) + CoocCFA(Dmin5,order,'hor',T,red);
 g.max41 = CoocCFA(Dmax5,order,'ver',T,red) + CoocCFA(Dmax5,order,'hor',T,red);

%**** FUNCTION ****%
function g = all3x3c(X,q,T,red)
% This function outputs co-occurrences of ALL residuals based on the
% KB kernel and its "halves" (EDGE residuals) as listed in Figure 1
% in our journal HUGO paper (version from June 14), including the naming
% convention.
...
for j=1:three
 ...
 g.spam11c = CoocCFA(Y,3,'col',T,red);
...
% spam14h,v  not bad, directional, hv-nonsym, to be symmetrized as spam
 g.spam14c = CoocCFA(vercat(Yu,Yb),3,'col',T,red) + CoocCFA(horcat(Yl,Yr),3,'col',T,red);
...
for j=1:three
 ...
 g.min24c = CoocCFA(vercat(vercat(Dmin1,Dmin2),vercat(Dmin3,Dmin4)),3,'col',T,red);
 g.max24c = CoocCFA(vercat(vercat(Dmax1,Dmax2),vercat(Dmax3,Dmax4)),3,'col',T,red);
    clear Dmin3 Dmin4 Dmax3 Dmax4
...
for j=1:three
 ...
 g.min22c = CoocCFA(UEq_min,3,'col',T,red) + CoocCFA(REq_min,3,'col',T,red);
 g.max22c = CoocCFA(UEq_max,3,'col',T,red) + CoocCFA(REq_max,3,'col',T,red);

% minmax41 -- good, non-directional, hv-sym, to be symmetrized as mnmx
for j=1:three
 [Dmin5{j},Dmax5{j}] = deal(min(Dmin1{j},Dmin2{j}),max(Dmax1{j},Dmax2{j}));
end,     clear Dmin1 Dmin2 Dmax1 Dmax2 
 g.min41c = CoocCFA(Dmin5,3,'col',T,red);
 g.max41c = CoocCFA(Dmax5,3,'col',T,red);
...
% spam14v  not bad, directional, hv-nonsym, to be symmetrized as spam
 g.spam14c = CoocCFA(horcat(Yu,Yb),3,'col',T,red) + CoocCFA(vercat(Yl,Yr),3,'col',T,red);
...
for j=1:three
 ...
 g.min24c = CoocCFA(horcat(horcat(Dmin1,Dmin2),horcat(Dmin3,Dmin4)),3,'col',T,red);      clear Dmin3 Dmin4 
 g.max24c = CoocCFA(horcat(horcat(Dmax1,Dmax2),horcat(Dmax3,Dmax4)),3,'col',T,red);      clear Dmax3 Dmax4
...
for j=1:three
 ...
 g.min22c = CoocCFA(UEq_min,3,'col',T,red) + CoocCFA(REq_min,3,'col',T,red);        clear UEq_min REq_min
 g.max22c = CoocCFA(UEq_max,3,'col',T,red) + CoocCFA(REq_max,3,'col',T,red);        clear UEq_max REq_max
...
%**** FUNCTION ****%
function f = CoocCFA(Res,order,type,T,red)
...
%         IIIIv = symm_sign(IIIIv,T,order); % symmetrization is postponed to post_processing
    ...
    case 'col'          % 3rd order coocs
        ...
        switch red      % sort the coulmns in this order: blue, green, green, red
            case 1
                f4 = f4(:,[4,3,2,1]);
            case 2
                f4 = f4(:,[2,1,4,3]);
